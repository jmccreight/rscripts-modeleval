...
if (strProc) {
	...
	if (reachRting) {
        ...
        if (strProcDaily) {
                baseStr <- as.data.frame(subset(modChrtout.d, modChrtout.d$tag=="Base"))
        } else {
                baseStr <- as.data.frame(subset(modChrtout, modChrtout$tag=="Base"))
        }
	stats_dates <- list()
	stats_qmean <- list()
	# Loop through runs
	for (j in 1:length(runTagList)) {
		...
		if (!is.null(stdate_stats_sub) & !is.null(enddate_stats_sub)) {
			...
			stats_str <- rbind(stats_str, results)
                ...
                # Stats against base case
		message("Stats against base")
		if (!(runTagList[[j]] == "Base")) {
                	results <- CalcStrStats(modFrxstout_tmp, baseStr, obsStrMeta, gageList,
                                        idCol.mod=idCol.mod, 
                                        stdate=stdate_stats, enddate=enddate_stats,
                                        parallel=parallelFlag)
               		results$tag <- runTag
                	results$seas <- "Full"
                	stats_str.base <- rbind(stats_str.base, results)
			if (!is.null(stdate_stats_sub) & !is.null(enddate_stats_sub)) {
                		results <- CalcStrStats(modFrxstout_tmp, baseStr, obsStrMeta, gageList,
                                        idCol.mod=idCol.mod, 
                                        stdate=stdate_stats_sub, enddate=enddate_stats_sub,
	...
	if (writeStatsFile) {
		...
		write.table(stats_str_tmp, file=paste0(writeDir, "/stats_str.txt"), sep="\t", row.names=FALSE)
                stats_str_tmp.base <- stats_str.base
                stats_str_tmp.base[is.na(stats_str_tmp.base)]<-(-1e+30)
                write.table(stats_str_tmp.base, file=paste0(writeDir, "/stats_str_base.txt"), sep="\t", row.names=FALSE)
...
saveList <- c(saveList, "stats_str", "stats_str.base", "stats_str_MEAN", "stats_dates", "stats_qmean")
}
