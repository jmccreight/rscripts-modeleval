...
if (strBiasMap) {
	...
	# Setup
	if (is.null(strBiasTags)) strBiasTags <- unique(stats_str.base$tag)
	if (is.null(strBiasSeas)) strBiasSeas <- unique(stats_str.base$seas)
	if (writeHtml) {
        	cat('## Streamflow Percent Change Maps\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
		strBias.ggList <- list()
...
# MIN STRFLOW Bias Maps
if (strminBiasMap) {
        message("Generating MIN STRFLOW change map...")
        # Setup
        if (is.null(strminBiasTags)) strminBiasTags <- unique(stats_str.base$tag)
        if (is.null(strminBiasSeas)) strminBiasSeas <- unique(stats_str.base$seas)
        if (writeHtml) {
                cat('## Min 10% Streamflow Change Maps\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                strminBias.ggList <- list()
                strminBias.freqList <- list()
                strminBias.histList <- list()
                strminBias.tblList <- list()
        }
        for (i in strminBiasTags) {
                for (j in strminBiasSeas) {
                        tbltmp <- subset(stats_str.base, stats_str.base$tag==i & stats_str.base$seas==j)
			tbltmp <- subset(tbltmp, tbltmp$site_no %in% forgages)
                        tbltmp <- plyr::join(tbltmp, subset(stats_qmean, stats_qmean$tag==i & stats_qmean$seas==j), by="site_no")
                        gg <- PlotMapErrors(geoMap, tbltmp,
                                plotTitle="Percent Change in Streamflow: Low Flows",
                                plotSubTitle=paste0(i, ", ", statsDateList_STR[[j]]),
                                sizeVar="qmean", colorVar="min10_bias",
                                sizeLab="Mean\nFlowrate\n(cms)", colorLab="Change (%)",
                                minThreshSize=0, maxThreshSize=150,
                                minThreshCol=(-100), maxThreshCol=100,
                                minPtsize=1.5, maxPtsize=6,
                                ...
                                colBreaks=divColRedYelBlu6,
                                valBreaks=c(-Inf, -25, -10, 10, 25, 100, Inf))
                        ggplot2::ggsave(filename=paste0(writePlotDir, "/strmin_bias_map_", i, "_", j, ".png"),
                                plot=gg[[1]], units="in", width=8, height=6, dpi=300)
                        ggplot2::ggsave(filename=paste0(writePlotDir, "/strmin_bias_hist_", i, "_", j, ".png"),
                                plot=gg[[3]], units="in", width=6, height=4, dpi=300)
                        if (writeHtml) {
                                strminBias.ggList <- c(strminBias.ggList, list(gg[[1]]))
                                strminBias.freqList <- c(strminBias.freqList, list(gg[[2]]))
                                strminBias.histList <- c(strminBias.histList, list(gg[[3]]))
                                if (statsMapTables) {
                                        tbltmp <- tbltmp[order(tbltmp$site_no),]
                                        tbltmp <- data.frame(site_no=tbltmp$site_no, lat=tbltmp$lat, lon=tbltmp$lon, n=tbltmp$t_n,
                                                bias=tbltmp$t_bias, mae=tbltmp$t_mae,
                                                corr=tbltmp$t_cor, daily_corr=tbltmp$dy_cor, mo_corr=tbltmp$mo_cor,
                                                nse=tbltmp$t_nse, daily_nse=tbltmp$dy_nse, mo_nse=tbltmp$mo_nse)
                                        strminBias.tblList <- c(strminBias.tblList, list(tbltmp))
                                }
                        }
                }
        }
        if (writeHtml) {
                for (i in 1:length(strminBias.ggList)) {
                        # Map
                        cat(paste0("```{r strminbiasmap_", i, ", fig.width = 8, fig.height = 6, out.width='800', out.height='600', echo=FALSE}\n"),
                                file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        plottxt <- knitr::knit_expand(text='strminBias.ggList[[{{i}}]]\n')
                        cat(plottxt, file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        cat('```\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        # Freq Histogram
                        cat(paste0("```{r strminbiashist_", i, ", fig.width = 6, fig.height = 4, out.width='600', out.height='400', echo=FALSE}\n"),
                                file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        plottxt <- knitr::knit_expand(text='strminBias.histList[[{{i}}]]\n')
                        cat(plottxt, file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        cat('```\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        # Table
                        if (statsMapTables) {
                                cat(paste0("```{r strminbiastbl_", i, ", fig.width = 8, fig.height = 6, out.width='800', out.height='600', echo=FALSE, results='asis'}\n"),
                                file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                                #tbltxt <- knitr::knit_expand(text='pandoc.table(strminBias.tblList[[{{i}}]], style = "simple", split.table=160)\n')
                                tbltxt <- knitr::knit_expand(text='print(xtable(strminBias.tblList[[{{i}}]]), type="html", comment=FALSE)\n')
                                cat(tbltxt, file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                                cat('```\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        }
                }
...
# MAX STRFLOW Bias Maps
if (strmaxBiasMap) {
        message("Generating MAX STRFLOW change map...")
        # Setup
        if (is.null(strmaxBiasTags)) strmaxBiasTags <- unique(stats_str.base$tag)
        if (is.null(strmaxBiasSeas)) strmaxBiasSeas <- unique(stats_str.base$seas)
        if (writeHtml) {
                cat('## Max 10% Streamflow Change Maps\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                strmaxBias.ggList <- list()
                strmaxBias.freqList <- list()
                strmaxBias.histList <- list()
                strmaxBias.tblList <- list()
        }
        for (i in strmaxBiasTags) {
                for (j in strmaxBiasSeas) {
                        tbltmp <- subset(stats_str.base, stats_str.base$tag==i & stats_str.base$seas==j)
			tbltmp <- subset(tbltmp, tbltmp$site_no %in% forgages)
                        tbltmp <- plyr::join(tbltmp, subset(stats_qmean, stats_qmean$tag==i & stats_qmean$seas==j), by="site_no")
                        gg <- PlotMapErrors(geoMap, tbltmp,
                                plotTitle="Percent Change in Streamflow: High Flows",
                                plotSubTitle=paste0(i, ", ", statsDateList_STR[[j]]),
                                sizeVar="qmean", colorVar="max10_bias",
                                sizeLab="Mean\nFlowrate\n(cms)", colorLab="Change (%)",
                                minThreshSize=0, maxThreshSize=150,
                                minThreshCol=(-100), maxThreshCol=100,
                                minPtsize=1.5, maxPtsize=6,
                                ...
                                colBreaks=divColRedYelBlu6,
                                valBreaks=c(-Inf, -25, -10, 10, 25, 100, Inf))
                        ggplot2::ggsave(filename=paste0(writePlotDir, "/strmax_bias_map_", i, "_", j, ".png"),
                                plot=gg[[1]], units="in", width=8, height=6, dpi=300)
                        ggplot2::ggsave(filename=paste0(writePlotDir, "/strmax_bias_hist_", i, "_", j, ".png"),
                                plot=gg[[3]], units="in", width=6, height=4, dpi=300)
                        if (writeHtml) {
                                strmaxBias.ggList <- c(strmaxBias.ggList, list(gg[[1]]))
                                strmaxBias.freqList <- c(strmaxBias.freqList, list(gg[[2]]))
                                strmaxBias.histList <- c(strmaxBias.histList, list(gg[[3]]))
                                if (statsMapTables) {
                                        tbltmp <- tbltmp[order(tbltmp$site_no),]
                                        tbltmp <- data.frame(site_no=tbltmp$site_no, lat=tbltmp$lat, lon=tbltmp$lon, n=tbltmp$t_n,
                                                bias=tbltmp$t_bias, mae=tbltmp$t_mae,
                                                corr=tbltmp$t_cor, daily_corr=tbltmp$dy_cor, mo_corr=tbltmp$mo_cor,
                                                nse=tbltmp$t_nse, daily_nse=tbltmp$dy_nse, mo_nse=tbltmp$mo_nse)
                                        strmaxBias.tblList <- c(strmaxBias.tblList, list(tbltmp))
                                }
                        }
                }
        }
        if (writeHtml) {
                for (i in 1:length(strmaxBias.ggList)) {
                        # Map
                        cat(paste0("```{r strmaxbiasmap_", i, ", fig.width = 8, fig.height = 6, out.width='800', out.height='600', echo=FALSE}\n"),
                                file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        plottxt <- knitr::knit_expand(text='strmaxBias.ggList[[{{i}}]]\n')
                        cat(plottxt, file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        cat('```\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        # Freq Histogram
                        cat(paste0("```{r strmaxbiashist_", i, ", fig.width = 6, fig.height = 4, out.width='600', out.height='400', echo=FALSE}\n"),
                                file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        plottxt <- knitr::knit_expand(text='strmaxBias.histList[[{{i}}]]\n')
                        cat(plottxt, file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        cat('```\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        # Table
                        if (statsMapTables) {
                                cat(paste0("```{r strmaxbiastbl_", i, ", fig.width = 8, fig.height = 6, out.width='800', out.height='600', echo=FALSE, results='asis'}\n"),
                                file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                                #tbltxt <- knitr::knit_expand(text='pandoc.table(strmaxBias.tblList[[{{i}}]], style = "simple", split.table=160)\n')
                                tbltxt <- knitr::knit_expand(text='print(xtable(strmaxBias.tblList[[{{i}}]]), type="html", comment=FALSE)\n')
                                cat(tbltxt, file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                                cat('```\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        }
                }
...
# STRFLOW COM Error Maps
if (strcomErrMap) {
        message("Generating STRFLOW COM error map...")
        # Setup
        if (is.null(strcomErrTags)) strcomErrTags <- unique(stats_str.base$tag)
        if (is.null(strcomErrSeas)) strcomErrSeas <- unique(stats_str.base$seas)
        if (writeHtml) {
                cat('## Error in COM of Streamflow Maps\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                strcomErr.ggList <- list()
                strcomErr.freqList <- list()
                strcomErr.histList <- list()
                strcomErr.tblList <- list()
        }
        for (i in strcomErrTags) {
                for (j in strcomErrSeas) {
                        tbltmp <- subset(stats_str.base, stats_str.base$tag==i & stats_str.base$seas==j)
			tbltmp <- subset(tbltmp, tbltmp$site_no %in% forgages)
                        tbltmp <- plyr::join(tbltmp, subset(stats_qmean, stats_qmean$tag==i & stats_qmean$seas==j), by="site_no")
                        gg <- PlotMapErrors(geoMap, tbltmp,
                                plotTitle="Error in the COM of Annual Streamflow",
                                plotSubTitle=paste0(i, ", ", statsDateList_STR[[j]]),
                                sizeVar="qmean", colorVar="yr_errcom",
                                sizeLab="Mean\nFlowrate\n(cms)", colorLab="Error\n(days)",
                                minThreshSize=0, maxThreshSize=150,
                                minThreshCol=(-100), maxThreshCol=100,
                                minPtsize=1.5, maxPtsize=6,
                                exclVar="dy_n", exclThresh=0.5*max(tbltmp$dy_n),
                                colBreaks=divColBrnWhtGrn5,
                                valBreaks=c(-Inf, -7, -2, 2, 7, Inf))
                        ggplot2::ggsave(filename=paste0(writePlotDir, "/strcom_err_map_", i, "_", j, ".png"),
                                plot=gg[[1]], units="in", width=8, height=6, dpi=300)
                        ggplot2::ggsave(filename=paste0(writePlotDir, "/strcom_err_hist_", i, "_", j, ".png"),
                                plot=gg[[3]], units="in", width=6, height=4, dpi=300)
                        if (writeHtml) {
                                strcomErr.ggList <- c(strcomErr.ggList, list(gg[[1]]))
                                strcomErr.freqList <- c(strcomErr.freqList, list(gg[[2]]))
                                strcomErr.histList <- c(strcomErr.histList, list(gg[[3]]))
                                if (statsMapTables) {
                                        tbltmp <- tbltmp[order(tbltmp$site_no),]
                                        tbltmp <- data.frame(site_no=tbltmp$site_no, lat=tbltmp$lat, lon=tbltmp$lon, n=tbltmp$t_n,
                                                bias=tbltmp$t_bias, mae=tbltmp$t_mae,
                                                corr=tbltmp$t_cor, daily_corr=tbltmp$dy_cor, mo_corr=tbltmp$mo_cor,
                                                nse=tbltmp$t_nse, daily_nse=tbltmp$dy_nse, mo_nse=tbltmp$mo_nse)
                                        strcomErr.tblList <- c(strcomErr.tblList, list(tbltmp))
                                }
                        }
                }
        }
        if (writeHtml) {
                for (i in 1:length(strcomErr.ggList)) {
                        # Map
                        cat(paste0("```{r strcomErrmap_", i, ", fig.width = 8, fig.height = 6, out.width='800', out.height='600', echo=FALSE}\n"),
                                file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        plottxt <- knitr::knit_expand(text='strcomErr.ggList[[{{i}}]]\n')
                        cat(plottxt, file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        cat('```\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        # Freq Histogram
                        cat(paste0("```{r strcomErrhist_", i, ", fig.width = 6, fig.height = 4, out.width='600', out.height='400', echo=FALSE}\n"),
                                file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        plottxt <- knitr::knit_expand(text='strcomErr.histList[[{{i}}]]\n')
                        cat(plottxt, file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        cat('```\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        # Table
                        if (statsMapTables) {
                                cat(paste0("```{r strcomErrtbl_", i, ", fig.width = 8, fig.height = 6, out.width='800', out.height='600', echo=FALSE, results='asis'}\n"),
                                file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                                #tbltxt <- knitr::knit_expand(text='pandoc.table(strcomErr.tblList[[{{i}}]], style = "simple", split.table=160)\n')
                                tbltxt <- knitr::knit_expand(text='print(xtable(strcomErr.tblList[[{{i}}]]), type="html", comment=FALSE)\n')
                                cat(tbltxt, file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                                cat('```\n', file=paste0(writePlotDir,"/plots_stats.Rmd"), append=TRUE)
                        }
                }
...
if (strCorrMap) {
	...
	# Setup
	if (is.null(strCorrTags)) strCorrTags <- unique(stats_str.base$tag)
	if (is.null(strCorrSeas)) strCorrSeas <- unique(stats_str.base$seas)
	if (writeHtml) {
